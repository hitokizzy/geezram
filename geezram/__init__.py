...
class GeezramConfig(BaseModel):
    """
    GeezRAM configuration class
    """
...
logging.basicConfig(
    ...
    level=logging.INFO)
logging.info("Starting geezram...")

if sys.version_info < (3, 8, 0):
    logging.error(
        "Your Python version is too old for geezram to run, please update to Python 3.8 or above"
    )
    exit(1)
...
try:
    CONFIG = GeezramConfig(**config_file)
...
try:
    ...
    CONFIG.telethon_client = TelegramClient("geezram", CONFIG.api_id,
                                            CONFIG.api_hash)

    # We import it now to ensure that all previous variables have been set
    from geezram.modules.helper_funcs.handlers import CustomCommandHandler, CustomMessageHandler
